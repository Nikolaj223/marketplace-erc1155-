...
export default function NFTCreation({ addNFTToList }) {
    ...
    const { data, isLoading: isMinting, isError, write } = useContractWrite({
        address: nftContract.address,
        abi: nftContract.abi,
        functionName: 'mint',
        onSuccess: (data) => {
            // Здесь можно вызвать addNFTToList
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)}>
            <TextField label="Token ID" {...register("tokenId", { required: "Token ID is required" })} error={!!errors.tokenId} helperText={errors.tokenId?.message} />
            <TextField label="Amount" type="number" {...register("amount", { required: "Amount is required" })} error={!!errors.amount} helperText={errors.amount?.message} />
            <TextField label="Metadata URL" {...register("metadataUrl", { required: "Metadata URL is required" })} error={!!errors.metadataUrl} helperText={errors.metadataUrl?.message} />
...
// import CardItem from './CardItem'; // Заменили NFTItem на CardItem

// export default function NFTCardCreation({ /* addNFTToList */ }) { // Убрали addNFTToList
//     const { register, handleSubmit, formState: { /* errors */ } } = useForm(); // Убрали errors
//     const { nftContract } = useContext(Web3Context);
...
//     return (
//         <CardItem title="Create NFT" description="Mint your own NFT" type="nft"> {/* Добавили type="nft" */}
//             <form onSubmit={handleSubmit(onSubmit)}>
